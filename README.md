...
Tracing, or mark and sweep garbage collection is a technique first implemented by McCarthy for the Lisp programming language in 1959. Mark and sweep GC allows a process to free all unreferenced objects by pausing the process and tracing the reference graph, marking any visited object as "in use", and then simply reclaiming the memory space of any object that is not marked. This incurs no overhead of keeping track of reference counts for each object in existence, and avoids the bugs possible from circular references.
