...
### Why manage memory?
In order the maintain a representation of the state of a process, the process will instruct the machine running it to save it's state in memory which is a fixed resource. Typically, when the state of the process no longer depends on any value saved in memory, that memory can be reclaimed in order to use it for a different purpose. This can be done manually by the author of the program. However, due to the large and complex nature of modern software, it is unreliable in many cases to depend on the author to manually manage the memory of a process. It is often more efficient and reliable to employ an automatic system which reclaims memory that is no longer in use.
